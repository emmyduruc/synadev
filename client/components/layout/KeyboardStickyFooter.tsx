...
/**
 * Pins footer actions directly above the software keyboard.
 * Use at the bottom of full-screen layouts with a flex content area.
 */
export const KeyboardStickyFooter = ({ children, gap = 12 }: KeyboardStickyFooterProps) => {
  const keyboardInset = useKeyboardInset();
  const { bottom: safeAreaBottom } = useSafeAreaInsets();
  const bottomSpacing =
    keyboardInset > 0 ? keyboardInset + gap : Math.max(safeAreaBottom, gap);
